# 俄罗斯方块重构  
<img src="https://s2.loli.net/2022/09/08/WFaMpre2dsI9H3D.png" width="60%">

---

## 📖 项目背景

五年前的作品，当我重新打开这个项目，它依然躺在硬盘角落。由于时间太长了，很多代码会报错，所以重新修订一遍。
...
| **复数运算协议** | **几何变换效果** | **物理动力学对应** |
| ---------------- | ---------------- | ------------------ |
| $z = x + yi$     | 确定初始相位     | 定域坐标捕获       |
| $z \times 1j$    | 逆时针旋转 90°   | 角速度恒定旋转     |
| $z \times (-1j)$ | 顺时针旋转 90°   | 镜像手性还原       |
| $z \times (-1)$  | 180° 中心对称    | 奇点生存转换       |

**逻辑注脚**：在 AFT 框架下，我们拒绝使用复杂的 `sin/cos` 浮点运算。通过复数域的离散映射，我们确保了在任何“逻辑纪元”下，方块的旋转永远是无损且精准的。
